...
export const fetchCountryData = async (countryName: string): Promise<CountryData> => {
    ...
    // Try to enhance with real-time data for major countries
    try {
        const basePopulation = COUNTRY_POPULATIONS[countryName] || mockData.population;
        const realTimeData = await realTimeDataService.getRealTimeData(countryName, basePopulation);
        
        return {
            ...mockData,
            realTimeData,
            isLiveData: false // Still using mock base data, but enhanced with real-time info
        };
    } catch (error) {
        console.log(`⚠️ Real-time data not available for ${countryName}, using base mock data`);
